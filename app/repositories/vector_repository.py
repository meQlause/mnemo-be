...
from app.utils import chunk_db
...
async def add_note_chunks(
    ...
    try:
        ...
        await chunk_db.bulk_save_chunks(session, chunks)
...
async def search_note_chunks_vector(
    ...
    """Calculates query embeddings and performs vector similarity search via utility."""
    embeddings_model = get_embeddings()
    ...
    return await chunk_db.run_vector_search_query(
        session=session,
...
async def reconstruct_note_context(
    ...
    """Groups matching chunks and fetches neighboring context window chunks via utility."""
    note_matches = {}
    note_objects = {}
    ...
    all_chunks = await chunk_db.get_chunks_by_note_ids(session, note_ids)
...
async def delete_chunks_by_note_id(session: AsyncSession, note_id: int) -> None:
    """Deletes all semantic chunks via utility."""
    await chunk_db.delete_chunks_for_note(session, note_id)
